...
norm_pack = c("knitr", "tidyverse", "BiocManager", "viridis", "RColorBrewer", "cowplot", "gridExtra", "tiff")
for (p in norm_pack) {
...
bioc_pack = c("SpatialExperiment", "dittoSeq", "flowCore", "scater", "BiocSingular", "bluster", "ConsensusClusterPlus")
for (p in bioc_pack) {
  if (!require(p, quietly = TRUE)) {
...
# Move distance measurements to the end of the dataframe
start_idx = which(names(cells) == "AreaShape_Center_Y")
end_idx = which(grepl("FullStack", names(cells)))[1] - 1
if (start_idx < end_idx) {
  cols_to_move = names(cells)[(start_idx + 1):end_idx]
  cells = cells %>%
    select(-all_of(cols_to_move), everything(), all_of(cols_to_move))
}
